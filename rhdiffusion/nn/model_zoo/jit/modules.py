...
from rhdiffusion.nn.model_zoo.jit.utils import rotate_half, repeat, pi, broadcat, modulate
...
class VisionRotaryEmbeddingFast(nn.Module):
    def __init__(
        ...
        if num_cls_token > 0:
            ...
            self.freqs_cos = freqs.cos().view(-1, freqs.shape[-1])
            self.freqs_sin = freqs.sin().view(-1, freqs.shape[-1])

    def forward(self, t): 

        if self.freqs_sin.device != t.device:
